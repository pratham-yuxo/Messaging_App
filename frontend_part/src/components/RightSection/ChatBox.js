...
const ChatBox = () => {
  ...
  useEffect(() => {
    const getMessages= async ()=>{
      let messages = await getConversation({senderId:Details.sub,receiverId:chatOfPersonOnWhichUHaveClicked.sub})
      // console.log(messages)
      setconversation(messages);
    }
    getMessages();
  }, [chatOfPersonOnWhichUHaveClicked.sub])
...
if (code===13) {
  ...
  if (!file) {
     ...
     message={
      senderId: Details.sub,
      receiverId:chatOfPersonOnWhichUHaveClicked.sub,
...
<Animated
  ...
  <MessageSection setmessageToDisplay={setmessageToDisplay} messageToDisplay={messageToDisplay} msgflag={msgflag} setmsgflag={setmsgflag} conversation={conversation} person={chatOfPersonOnWhichUHaveClicked}/>
  <Animated
